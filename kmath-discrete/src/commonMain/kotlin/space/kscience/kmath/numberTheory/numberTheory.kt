 ...
 */
tailrec fun gcd(a: Int, b: Int): Int = if (a == 0) abs(b) else gcd(b % a, b)

/**
 * Computes [Greatest Common Divisor](https://en.wikipedia.org/wiki/Greatest_common_divisor) of the [values].
 */
fun gcd(vararg values: Int): Int = values.reduce(::gcd)
fun gcd(values: Iterable<Int>): Int = values.reduce(::gcd)
 ...
 * Hence its time complexity is $$O(\log(a+b))$$ (see [Wolfram MathWorld](https://mathworld.wolfram.com/EuclideanAlgorithm.html)).
 */
tailrec fun gcd(a: Long, b: Long): Long = if (a == 0L) abs(b) else gcd(b % a, b)

/**
 * Computes [Greatest Common Divisor](https://en.wikipedia.org/wiki/Greatest_common_divisor) of the [values].
 */
fun gcd(vararg values: Long): Long = values.reduce(::gcd)
fun gcd(values: Iterable<Long>): Long = values.reduce(::gcd)

/**
 * Computes "the smallest" [Bézout coefficients](https://en.wikipedia.org/wiki/B%C3%A9zout%27s_identity) and
 * [GCD](https://en.wikipedia.org/wiki/Greatest_common_divisor) of [a] and [b].
 */
fun bezoutIdentityWithGCD(a: Long, b: Long): BezoutIdentityWithGCD<Long> =
    when {
        a < 0 && b < 0 -> with(bezoutIdentityWithGCDInternalLogic(-a, -b, 1, 0, 0, 1)) { BezoutIdentityWithGCD(-first, -second, gcd) }
        a < 0 -> with(bezoutIdentityWithGCDInternalLogic(-a, b, 1, 0, 0, 1)) { BezoutIdentityWithGCD(-first, second, gcd) }
        b < 0 -> with(bezoutIdentityWithGCDInternalLogic(a, -b, 1, 0, 0, 1)) { BezoutIdentityWithGCD(first, -second, gcd) }
 ...
 * Test for not primality of [this].
 */
fun Long.isNotPrime(): Boolean = !isPrime()

/**
 * Computes [Kronecker symbol](https://en.wikipedia.org/wiki/Kronecker_symbol) of entries [a] and [b].
 */
@Suppress("NAME_SHADOWING")
fun kroneckerSymbol(a: Long, b: Long): Int {
    // Simple cases: when
    // `b` is `0`,
